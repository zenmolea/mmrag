...
whisper_model = WhisperForConditionalGeneration.from_pretrained("whisper-large", torch_dtype=torch.float16, device_map="auto")
...
for item in tqdm(mme_data[index:], desc="Processing items"):
    ...
    for q_num, question in enumerate(content['questions']):
        ...
        res, token_num = llava_inference(qs, video)
